"""WebSocket-based Who Am I bot simulator.

Usage:
  python -m app.tests.who_am_i_ws_bot_simulator --base-url http://127.0.0.1:8000 --sessions 20 --players 6 --workers 5
"""
...
class WSPlayerClient:
    ...
    def _next_action_id(self) -> str:
        ...
        return f"who-ws-{self.player_id[:6]}-{int(time.time() * 1000)}-{self._action_counter}"
...
async def _resolve_identity_guess_async(
    base_url: str,
    room_code: str,
...
async def run_ws_session(
    ...
    try:
        status, cat_data = await asyncio.to_thread(_http_json, base_url, "GET", "/api/who-am-i/categories")
        if status != 200 or not isinstance(cat_data, dict):
            raise RuntimeError(f"load categories failed: status={status} data={cat_data}")
        categories_map = cat_data.get("categories") or {}
        if not isinstance(categories_map, dict) or not categories_map:
            raise RuntimeError("no who-am-i categories returned")
        chosen_categories = list(categories_map.keys())[: min(4, len(categories_map))]

        host_name = f"who_ws_host_{session_index}_{_rand_suffix(4)}"
        create_payload = {
            "host_name": host_name,
            "character_id": "char1",
        ...
        status, create_data = await asyncio.to_thread(
            _http_json, base_url, "POST", "/api/who-am-i/rooms", create_payload
        )
        ...
        for i in range(2, players + 1):
            join_name = f"who_ws_{session_index}_{i}_{_rand_suffix(3)}"
            payload = {"player_name": join_name, "character_id": "char1"}
            status, join_data = await asyncio.to_thread(
                _http_json, base_url, "POST", f"/api/who-am-i/rooms/{room_code}/join", payload
            )
        ...
        for pid in player_ids:
            ws_url = f"{ws_base}/api/who-am-i/ws/{room_code}?player_id={pid}"
        ...
        for step in range(max_steps):
            ...
            if not isinstance(state, dict):
                status, polled = await asyncio.to_thread(
                    _http_json, base_url, "GET", f"/api/who-am-i/rooms/{room_code}"
                )
            ...
            if state.get("ended"):
                ...
                solved = sum(1 for p in (state.get("players") or []) if p.get("has_guessed_correctly"))
            ...
            if state.get("reveal_phase_active"):
                await id_to_client[host_id].send_action("confirm_reveal", {})
                continue

            turn_player_id = str(state.get("current_turn_player_id") or "")
            if not turn_player_id or turn_player_id not in id_to_client:
                await asyncio.sleep(0.03)
                continue

            guess_text = await _resolve_identity_guess_async(
                base_url=base_url,
                room_code=room_code,
                room_state=state,
                target_player_id=turn_player_id,
                host_id=host_id,
            )
            try:
                await id_to_client[turn_player_id].send_action(
                    "submit_guess",
...
async def run_all(args: argparse.Namespace) -> int:
    ...
    print(
        ...
        f"players={args.players} workers={args.workers} elapsed={elapsed:.2f}s"
    )
    return 0 if fail_count == 0 else 1


def main() -> int:
    parser = argparse.ArgumentParser(description="Run WebSocket-based Who Am I bot sessions.")
    ...
    parser.add_argument("--max-steps", type=int, default=500, help="Max loop steps per session")
