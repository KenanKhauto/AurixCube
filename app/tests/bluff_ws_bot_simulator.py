"""WebSocket-based Bluff bot simulator.
...
Usage:
  python -m app.tests.bluff_ws_bot_simulator --base-url http://127.0.0.1:8000 --sessions 20 --players 6 --rounds 6 --workers 5
"""
...
class WSPlayerClient:
    ...
    def _next_action_id(self) -> str:
        ...
        return f"ws-act-{self.player_id[:6]}-{int(time.time() * 1000)}-{self._action_counter}"
    ...
    async def _recv_loop(self) -> None:
        ...
        try:
            ...
            # Receiver exiting is handled by send timeouts/close.
            pass
        except asyncio.CancelledError:
            # Expected when close() cancels the receiver task.
            pass
...
async def run_ws_session(
    ...
    try:
        status, cat_data = await asyncio.to_thread(_http_json, base_url, "GET", "/api/bluff/categories")
        ...
        host_name = f"ws_host_{session_index}_{_rand_suffix(4)}"
        create_payload = {
            "host_name": host_name,
            "character_id": "char1",
        ...
        status, create_data = await asyncio.to_thread(_http_json, base_url, "POST", "/api/bluff/rooms", create_payload)
        ...
        for i in range(2, players + 1):
            join_name = f"ws_{session_index}_{i}_{_rand_suffix(3)}"
            payload = {"player_name": join_name, "character_id": "char1"}
            status, join_data = await asyncio.to_thread(
                _http_json, base_url, "POST", f"/api/bluff/rooms/{room_code}/join", payload
            )
        ...
        for step in range(max_steps):
            ...
            if not isinstance(state, dict):
                # Fallback poll on stalled state
                status, polled = await asyncio.to_thread(_http_json, base_url, "GET", f"/api/bluff/rooms/{room_code}")
            ...
            if phase == "category_pick":
                chooser_id = str(state.get("current_category_chooser_id") or "")
                if chooser_id in id_to_client:
                    allowed = state.get("categories") or chosen_categories
                    category = random.choice(allowed)
                    await id_to_client[chooser_id].send_action("select_category", {"category": category})
                continue

            if phase == "submission":
                submitted = set(state.get("submitted_player_ids") or [])
                players_state = state.get("players") or []
                for p in players_state:
                    pid = str(p.get("id"))
                    if pid in submitted or pid not in id_to_client:
                        continue
                    try:
                        await id_to_client[pid].send_action(
                            "submit_answer",
                            {"answer_text": f"ws-answer-{_rand_suffix(8)}"},
                            timeout=5.0,
                        )
                    except Exception:
                        # Phase may have moved; continue.
                        pass
                continue

            if phase == "answer_pick":
                picked = set(state.get("picked_player_ids") or [])
                options = state.get("answer_options") or []
                players_state = state.get("players") or []
                for p in players_state:
                    pid = str(p.get("id"))
                    if pid in picked or pid not in id_to_client:
                        continue
                    valid = [o for o in options if pid not in set(o.get("author_ids") or [])]
                    if not valid:
                        continue
                    option = random.choice(valid)
                    try:
                        await id_to_client[pid].send_action(
                            "submit_pick",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run WebSocket-based Bluff bot sessions.")
    ...
    parser.add_argument("--rounds", type=int, default=4, help="Rounds per session (>= players)")
    parser.add_argument("--workers", type=int, default=4, help="Parallel sessions")
    parser.add_argument("--max-steps", type=int, default=400, help="Max phase loop steps per session")
    args = parser.parse_args()

    if args.players < 2 or args.players > 10:
        raise SystemExit("--players must be between 2 and 10")
    if args.rounds < args.players:
        raise SystemExit("--rounds must be >= --players")
    if args.sessions < 1:
